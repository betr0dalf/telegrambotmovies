...
movies['ratingKP'] = pd.to_numeric(movies['ratingKP'], errors='coerce').fillna(0)
...
def on_click(message):
    try:
        if message.text == 'Искать фильм 🍿':
            ...
            msg = bot.send_message(message.chat.id, 'Введите год выхода фильмов:')
            ...
            bot.send_message(message.chat.id, 'Нажмите на любую из кнопок внизу экрана 👁️👄👁️')
            show_main_menu(message)
...
def search_movie(message):
    ...
    found_movies = movies[movies['NameFilm'].str.contains(movie_name, case=False, na=False)]
    if found_movies.empty:
        msg = bot.send_message(message.chat.id, 'Фильм не найден. Попробуйте ещё раз:')
        bot.register_next_step_handler(msg, search_movie)
    else:
...
def top_movies_by_director(message, director):
    director_movies = movies[movies['Directors'].str.contains(director, case=False, na=False)]
    top_movies = director_movies.nlargest(10, 'ratingKP')

    if top_movies.empty:
        bot.send_message(message.chat.id, f'Фильмы режиссёра {director} не найдены.')
        show_main_menu(message)
    else:
...
def top_movies_by_actor(message, actor):
    actor_movies = movies[movies['Actors'].str.contains(actor, case=False, na=False)]
    top_movies = actor_movies.nlargest(10, 'ratingKP')

    if top_movies.empty:
        bot.send_message(message.chat.id, f'Фильмы с актёром {actor} не найдены.')
        show_main_menu(message)
    else:
...
def send_top_movies(message, top_movies, category):
    try:
        ...
        for _, row in top_movies.iterrows():
            button = types.InlineKeyboardButton(f"{row['NameFilm']}", callback_data=f"movie_{row['KPid']}")
            markup.add(button)
        bot.send_message(message.chat.id, 'Выберите фильм для подробностей:', reply_markup=markup)
    except Exception as e:
        bot.send_message(message.chat.id, f'Произошла ошибка при формировании списка фильмов: {e}')
        show_main_menu(message)


def show_movie_details(message, movie):
    try:
        bot.send_message(message.chat.id, f"{movie['NameFilm']} ({int(movie['Year'])})\n"
        ...
        markup = types.InlineKeyboardMarkup()
        watch_button = types.InlineKeyboardButton('Смотреть фильм', url=movie['WebUrl'])
        kp_button = types.InlineKeyboardButton('Страница на Кинопоиске', url=movie['WebUrl2'])
        markup.add(watch_button)
        markup.add(kp_button)
        bot.send_message(message.chat.id, 'Выберите действие:', reply_markup=markup)
    except Exception as e:
        bot.send_message(message.chat.id, f'Произошла ошибка при показе информации о фильме: {e}')
    show_main_menu(message)
